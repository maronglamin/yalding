...
function table_td($arr_data, $index=[]) 
{
    $string = '';
    $lastIndex = end($index);
    while($result = mysqli_fetch_assoc($arr_data)) 
    {
        foreach($index as $val) 
        {
            ...
            {
                $id = $result['stud_id'];
                $string .= '<td><a href="'.PROOT.'admin/components/admission_details.php?stud_id='. $id. '" class="btn btn-primary">Details</a></td>';
            } else {
...
function html_table($th=[],$queryStatement, $index=[]) {
    $html = '<table class="table table-hover personal-task"><thead>';
    $html .=  table_th($th);
    $html .= '</thead>';
    $html .= '<tbody>';
    $html .= table_td($queryStatement,$index);
    $html .= '</tbody></table>';
    $html .= '</section></div>';
    return $html;
}